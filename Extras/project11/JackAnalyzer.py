...
class CompilationEngine:
    ...
    def compile_subroutine(self):
        ...
        local_args = self.symbol_table.var_count("local")
        self.vm_writer.write_function(subroutine_name, local_args)

        if subroutine_kind == "constructor":
            fields = self.symbol_table.var_count("field")
            self.vm_writer.write_push("constant", fields)
            self.vm_writer.write_call("Memory.alloc", 1)
            self.vm_writer.write_pop("pointer", 0)  # puts this in pointer 0
    ...
    def compile_subroutine_call(self):
        ...
        self.token_stream.advance()
        if self.token_stream.identifier() == ".":  # className.functionName
            L0X = False
            self.token_stream.advance()
...
#functionName, ClassName.functionName, ClassName.new Object.functionName,
    ...
    def compile_term(self):
        ...
        if self.token_stream.isInt():
            ...
            print(self.token_stream.getToken())
    ...
    def build_string(self, string):
        ...
        for c in string:
            ...
            self.vm_writer.write_call("String.appendChar", 2)     # appending the char to the string
